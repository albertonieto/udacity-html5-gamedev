...
SpriteSheetClass = Class.extend({
	...
	parseAtlasDefinition: function (atlasJSON) {
        ...
        //
        // YOUR CODE HERE
        for(var name in atlasParsed.frames){
            var atlasFrames = atlasParsed.frames;
            var x = atlasFrames[name].frame.x,
                y = atlasFrames[name].frame.y,
                w = atlasFrames[name].frame.w,
                h = atlasFrames[name].frame.h,
                cx = -1*Math.round(atlasFrames[name].frame.w/2),
                cy = -1*Math.round(atlasFrames[name].frame.h/2);
...
function drawSprite(spritename, posX, posY) {
    ...
    for(var spritesheetname in gSpriteSheets){
        spriteSheetInstance = gSpriteSheets[spritesheetname];

        spriteInstance = spriteSheetInstance.getStats(spritename);
        // sprite with name spritename was found on the sheet
        if(spriteInstance){
             __drawSpriteInternal(spriteInstance, spriteSheetInstance, posX, posY);
        }
    }
